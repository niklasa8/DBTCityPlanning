...
alpha = handles.alpha;
beta = handles.beta;
carMulti = 1;   % Faktor för att ändra bilhastighet.
...
for i = 1:n %Plottar itersection nodes och färglägger dem enligt snabbaste färdsättet.
    ...
    if handles.travBike && handles.travCar && handles.travBus
        ...
        if carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) ~= inf
            if bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) < carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60) && bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) < fastest_trip(i)
                set(handles.ch(n_ways + nr + n - i + 1),'color','r','visible','on');
            end
        end

        if carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60) < bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) && carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60) < fastest_trip(i)
            set(handles.ch(n_ways + nr + n - i + 1),'color','g','visible','on');
        end

        if carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) ~= inf
            if fastest_trip(i) < bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) && fastest_trip(i) < carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60)
    ...
    elseif handles.travBike && handles.travCar
        % lägger på tio procent på bilens tid
        if carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) ~= inf
            if bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) < carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60)
                set(handles.ch(n_ways + nr + n - i + 1),'color','r','visible','on');
            end
        end

        if carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60) < bicycling_path(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source)))
            set(handles.ch(n_ways + nr + n - i + 1),'color','g','visible','on');
        end
    ...
    elseif handles.travBus && handles.travCar
        % lägger på tio procent på bilens tid
        if carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60) < fastest_trip(i)
            set(handles.ch(n_ways + nr + n - i + 1),'color','g','visible','on');
        end

        if carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) ~= inf
            if fastest_trip(i) < carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60)
                 set(handles.ch(n_ways + nr + n - i + 1),'color','b','visible','on');
            end
        end
    ...
    elseif handles.travCar
        ...
        cartime = carMulti*(carTo_walkFrom_parking(i,handles.graph_data.intnd_map(handles.graph_data.id_map(node_source))) + 3/60);
