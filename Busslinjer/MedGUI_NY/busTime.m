function fastest_trip = busTime(node_source,graph_data,time,day,generalCost,alphaPar,betaPar)
...
% Parametrar om generell kostnad används
if generalCost
	walkCost = 135;
	busCost = 33;
	ticketCost = 25;
...
dest = graph_data.intnd_map(graph_data.id_map(node_source));    % Destinationsnod
t = time;   % Angiven tid man vill vara vid destinationsnod

% Dagparameter: 1(Mån-Tors),2(Fre),3(Lör),4(Sön)
...
n_stops = max(size(bus_stop_nodes)); % Antal busshållplatser
