...
%         end
    ...
    if mod(t,1) == 0 %Spawn new car.
        ...
        stop = 0;
        while stop == 0
            p_spawn_residential = 0.4;
            p_spawn_city = 0.3;
            p_spawn_influx = 0.3;
            p_dest_residential = 0.4;
            p_dest_city = 0.3;
            p_dest_influx = 0.3;
...
for i = 1:n_edges
    ...
    if CGM_sparse(edge(i).from,edge(i).to) ~= 0 && CGM_sparse(edge(i).from,edge(i).to) ~= inf && ~isempty(edge(i).time_on_edge)
        CGM_sparse(edge(i).from,edge(i).to) = mean(CGM_sparse(edge(i).from,edge(i).to) + edge(i).time_on_edge./3600);
    end
end
...
car_all_shortest_path = graphallshortestpaths(CGM_sparse);
...
save('graph_data.mat','CGM_sparse','car_all_shortest_path','carTo_walkFrom_parking','-append')
end