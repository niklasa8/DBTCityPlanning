function found_way_id = findWay(graph_data, data_umea, input_data, waytype)
...
if strcmp(waytype,'defualt')
    ...
    elseif length(possible_ways) == 1
        %Om det bara fanns en möjlig väg så returnar vi den direkt
        %(denna stämmer med all sannolikhet)
        waitbar(1,h);
        delete(h)

        found_way_id = possible_ways(1); %OBS ger pos. i ways EJ OSM ID
    end

% end of default way

elseif strcmp(waytype,'edited')
    len_input = length(input_data);
    len_node = length(node);
    waitbar(0.25,h);
    possible_nodes = {};
    for i=1:len_input %söker igenom "arrayen"
        lon = input_data(i); %lon är på första 1 till N
        ...
        for j=1:len_node %Kollar igenom hela intnd, sparar undan positionen i intnd fï¿½r match
            if node(j).lon == lon && node(j).lat == lat;
                aaa = ismember(possible_nodes,node(j).id);
                if all(aaa == 0)
                    possible_nodes = [possible_nodes, node(j).id];
                    waitbar(0.5,h);
                end
            end
        end
    ...
    waitbar(0.75,h);
    for i=1:length(possible_nodes)
        possible_nodes_cell{i} = str2double(possible_nodes{i});
    end
    found_way_id = possible_nodes_cell;
    waitbar(1,h);
    delete(h)
end
