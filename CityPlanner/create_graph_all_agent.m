%function create_graph_all_agent(data_umea)
...
%way = data.umea_way;
%config_graph = data_umea.config_graph;
    ...
    for j = 2:n_ndref %Loopa över alla noder som way(i) refererar till, förutom första noden eftersom den alltid sparas.
        ...
        if (node(id_map(way(i).ndref{j})).waynd > 1) || j == n_ndref || ~isempty(node(id_map(way(i).ndref{j})).parking) || (node(id_map(way(i).ndref{j})).bus_stop == 1) %Om noden som kontrolleras refereras från flera vägar eller om det är sista noden vägen refererar till är det en intersection node.
            ...
            for h = k:j-1 %Loop som räknar ut längden mellan nuvarande nod från tidigare sparad nod på vägen.
                ...
                length1 = length1 + latlon2meters(lat1,lat2,lon1,lon2);
            ...
            if way(i).highway && way(i).cycleway == 0 && way(i).bus_only == 0 %Om vägen inte är en gångväg eller cykelväg är det en bilväg.
                ...
                smallConst = 0.000001;  % förhindrar att två sammankopplade noder får värdet 0 i matrisen, om längden mellan är 0.
                weight = 10^-3*length1/max_speed + traffic_calming*time_delay_TC/3600 + traffic_signals*time_delay_TS/3600 + smallConst; %Vikt på kanten mellan nod j och k.
                graph_file.Car_graph_matrix(intnd_map(id_map(way(i).ndref{k}),1),intnd_map(id_map(way(i).ndref{j}),1)) = weight;
                graph_file.Bus_graph_matrix(intnd_map(id_map(way(i).ndref{k}),1),intnd_map(id_map(way(i).ndref{j}),1)) = weight;
                graph_file.Car_dist_matrix(intnd_map(id_map(way(i).ndref{k}),1),intnd_map(id_map(way(i).ndref{j}),1)) = 10^-3*length1 + smallConst;

                edge(edge_count).dist = length1;
                ...
                if way(i).oneway == 0 %Om vägen inte är enkelriktad skall även det speglade elementet få samma vikt. Detta är också en förenkling eftersom det kan vara mer lättframkomligt åt ena hållet på en väg.
                    ...
                    graph_file.Car_dist_matrix(intnd_map(id_map(way(i).ndref{j}),1),intnd_map(id_map(way(i).ndref{k}),1)) = 10^-3*length1 + smallConst;
            ...
            if way(i).highway && way(i).cycleway == 0 && way(i).bus_only == 1
                ...
                weight = 10^-3*length1/max_speed + traffic_calming*time_delay_TC/3600 + traffic_signals*time_delay_TS/3600 + smallConst;
                graph_file.Bus_graph_matrix(intnd_map(id_map(way(i).ndref{j}),1),intnd_map(id_map(way(i).ndref{k}),1)) = weight;
            end

            %if way(i).bicycle || way(i).cycleway %Om man får cykla på vägen antas även att man får gå på den.
                weight = 10^-3*length1/ + smallConst; %Vikt på kanten mellan nog j och k.
            ...
            %if way(i).bicycle %Väg man får cykla på.
                weight = 10^-3*length1/cyclingspeed + smallConst; %Vikt på kanten mellan nog j och k.
...
save('graph_data.mat','walk_path_fromParking','carTo_walkFrom_parking','bicycling_path','intnd_map','id_map','intnd_count','intnd','walk_all_shortest_path','edge','car_all_shortest_path','edge_index','CGM_sparse','car_dist_shortest_path')
